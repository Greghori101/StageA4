            ...
            @foreach($favorites as $modelType => $items)
                <li class="nav-item" role="presentation">
                    <button class="nav-link {{ $loop->first ? 'active' : '' }}" 
                        id="tab-{{ Str::slug($modelType) }}" 
                        data-bs-toggle="tab" 
                        data-bs-target="#content-{{ Str::slug($modelType) }}" 
                        type="button" 
                        role="tab" 
                        aria-controls="content-{{ Str::slug($modelType) }}" 
        ...
        <div class="tab-content mt-3" id="favoriteTabsContent">
            @foreach($favorites as $modelType => $items)
                <div class="tab-pane fade {{ $loop->first ? 'show active' : '' }}" 
                    ...
                    role="tabpanel" 
                    aria-labelledby="tab-{{ Str::slug($modelType) }}">
                        ...
                        <table class="table table-bordered">
                            ...
                            <tbody>
                                @foreach($items as $favorite)
                                    @php $model = app($favorite->model_type)::find($favorite->model_id); @endphp
                                    @if($model)
                                        <tr>
